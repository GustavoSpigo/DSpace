...
<h2>Vocabulário Controlado RIC-CPS</h2>
<strong>Vocabulário Controlado RIC-CPS</strong> 
é uma lista de assuntos (palavras-chave) utilizada para a indexação 
de recursos de informação, existentes no acervo do RIC-CPS. <br> <br>
O Vocabulário Controlado do RIC-CPS é baseado no Vocabulário Controlado do Sistema Dedalus – SIBI – USP 
e a inserção de novos termos deve passar pela aprovação do Grupo Trabalho (GT) do Vocabulário Controlado, 
composto por bibliotecários das unidades do CPS com a gestão do CGD – Centro de Gestão Documental, 
através da opção "Vocabulário Controlado" no menu. <br> <br>

<% if (user == null) { %>
  <iframe id="ifrm-spigo" src="https://apps.spigo.net/vocabulario-controlado/solicitacao" style="    width: 100%;height: 70vh;border: none;"> </iframe>
<% }else{ %>
  <iframe id="ifrm-spigo" src="https://apps.spigo.net/vocabulario-controlado/solicitacao?nome=<%=user.getFullName()%>&mail=<%=user.getEmail()%>" style="    width: 100%;height: 70vh;border: none;" > </iframe>
<% } %>
